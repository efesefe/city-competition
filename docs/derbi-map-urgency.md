...
`cities-fill` → **`cities-stripes-{tribeId}`** → `cities-outline` → `cities-tension-ring` (contest tension, under derby) → `cities-derbi-glow` → `cities-selected` → `cities-ticker-highlight` (brief ticker pulse) → labels → crests → HTML overlay (derby badge + chip, momentum badges)
