...
func newSocialMux(t *testing.T, pool *pgxpool.Pool) (*http.ServeMux, *auth.SessionService) {
	...
	mux.Handle("POST /v1/friends/requests", auth.RequireSession(sessions, http.HandlerFunc(h.CreateFriendRequest)))
	mux.Handle("GET /v1/friends/requests", auth.RequireSession(sessions, http.HandlerFunc(h.ListFriendRequests)))
	mux.Handle("POST /v1/friends/requests/{id}/accept", auth.RequireSession(sessions, http.HandlerFunc(h.AcceptFriendRequest)))
	mux.Handle("POST /v1/friends/requests/{id}/reject", auth.RequireSession(sessions, http.HandlerFunc(h.RejectFriendRequest)))
	mux.Handle("DELETE /v1/friends/requests/{id}", auth.RequireSession(sessions, http.HandlerFunc(h.CancelFriendRequest)))
	mux.Handle("GET /v1/friends", auth.RequireSession(sessions, http.HandlerFunc(h.ListFriends)))
	mux.Handle("DELETE /v1/friends/{user_id}", auth.RequireSession(sessions, http.HandlerFunc(h.Unfriend)))
	mux.Handle("POST /v1/blocks", auth.RequireSession(sessions, http.HandlerFunc(h.CreateBlock)))
	mux.Handle("GET /v1/blocks", auth.RequireSession(sessions, http.HandlerFunc(h.ListBlocks)))
	mux.Handle("DELETE /v1/blocks/{user_id}", auth.RequireSession(sessions, http.HandlerFunc(h.DeleteBlock)))
	mux.Handle("POST /v1/mutes", auth.RequireSession(sessions, http.HandlerFunc(h.CreateMute)))
	mux.Handle("GET /v1/mutes", auth.RequireSession(sessions, http.HandlerFunc(h.ListMutes)))
	mux.Handle("DELETE /v1/mutes/{user_id}", auth.RequireSession(sessions, http.HandlerFunc(h.DeleteMute)))
	mux.Handle("POST /v1/reports", auth.RequireSession(sessions, http.HandlerFunc(h.CreateReport)))
	return mux, sessions
}
